...
`FROM .ds-logs-* | WHERE event_source == "GreenCoat" | STATS COUNT(*) BY event.url`
...
The Windows event ID for user account creation is 4720. This query counts such events and returns 0 when executed:  
...
The domain controller's FQDN is `"dc01.northpole.local` and the LDAP unencrypted port is 389. Using this filter and sorting the events by timestamp, the earliest such event can be identified.  
...
For this question, the Linux command line tool `grep` seems more effective in exploring the filtered events. The two log files first need to be combined into one (in this case, they are combined into `log_chunk_all.log`). Then two case-insensitive grep searches using the attacker's username `nutcrackr` and `powershell` are performed:  
